...
import { FaPhoneAlt, FaMapMarkerAlt, FaCalendarAlt, FaRegCheckCircle } from "react-icons/fa";
...
const ImageCarousel = ({ images = [] }) => {
  ...
  useEffect(() => {
    ...
    const onPointerDown = (e) => {
      ...
      const item = e.target.closest ? e.target.closest(".carousel-item") : null;
    ...
    const onPointerUp = (e) => {
      ...
      if (!isDraggingRef.current && candidateIndexRef.current >= 0 && Math.abs(dx) < threshold && Math.abs(dy) < threshold) {
        ...
        // debug: console.log("Open lightbox index:", candidateIndexRef.current);
      }
  ...
  return (
    <>
      <div className="carousel-root" aria-label="Image slider">
        {showArrows && (
          <button className={`carousel-arrow carousel-arrow-left ${canScrollLeft ? "" : "hidden"}`} onClick={() => scrollByOneItem("left")}>
            <FaChevronLeft />
          </button>
        )}

        <div className="carousel-viewport">
          <div className="carousel-track" ref={trackRef} role="list" style={{ touchAction: "pan-y" }}>
            {images.map((src, i) => (
        ...
        {showArrows && (
          <button className={`carousel-arrow carousel-arrow-right ${canScrollRight ? "" : "hidden"}`} onClick={() => scrollByOneItem("right")}>
            <FaChevronRight />
          </button>
        )}
      </div>
      ...
      {isLightboxOpen && (
        <div className="ic-lightbox-overlay" onClick={closeLightbox}>
          <div className="ic-lightbox-content" onClick={(e) => e.stopPropagation()}>
            <button className="ic-lightbox-close" onClick={closeLightbox}><IoClose size={28} /></button>
            <button className="ic-lightbox-prev" onClick={prevImage}><FaChevronLeft /></button>
            ...
            <button className="ic-lightbox-next" onClick={nextImage}><FaChevronRight /></button>
          </div>
        </div>
      )}
    </>
  ...
  useEffect(() => {
    ...
    const onWheelCapture = (e) => {
      ...
      if (panel.contains(e.target)) {
        ...
        const atBottom =
          panel.scrollHeight - panel.clientHeight === panel.scrollTop && e.deltaY > 0;
    ...
    const onTouchMoveCapture = (e) => {
      ...
      if (panel.contains(e.target)) {
        ...
        const atBottom =
          panel.scrollHeight - panel.clientHeight === panel.scrollTop && deltaY < 0;
    ...
    <div className="video-detail-overlay" onClick={onClose}>
      <div
        ...
        onTouchStart={(e) => {
          ...
          if (e.touches && e.touches[0]) touchStartYRef.current = e.touches[0].clientY;
        }}
          ...
          {loading ? (
            ...
            <>
              <div className="video-detail-content">
                  ...
                  <>
                    <div className="seller-info">
                      ...
                      <div className="seller-details">
                        <div className="seller-name">{data.nguoiDang.fullName}</div>
                      </div>
                    </div>
                ...
                {data.danhSachAnh?.length > 0 && <ImageCarousel images={data.danhSachAnh} />}


                <div className="price">
                  <span>{data.gia?.toLocaleString()} đ</span>
                  {data.coTheThoaThuan && <span className="note">(Có thể thương lượng)</span>}
                </div>
              ...
              {data.moTa && (
                <div className="description-section">
                  <h5 className="description-title">Mô tả chi tiết</h5>
                  <p className={`description-text ${expanded ? "expanded" : "collapsed"}`}>
                    {data.moTa}
                  </p>
                  {data.moTa.length > 84 && (
